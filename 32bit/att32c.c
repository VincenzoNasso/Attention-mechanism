...
void estraiFetta(int indice, params *input, MATRIX ret){
	int c1 = 0;
	int inizio = indice * input->n * input->d;
...
void att(params *input){
	...
	int inizioFetta = 0;
	int numeroDiFette = input->ns * input->s;
	...
	for (int i = 0; i < numeroDiFette; i++){
		estraiFetta(i, input, fetta);
		
		// n, d, nn devono essere multipli di 16
		...
		if(input->n % 16 == 0 && input->d % 16 == 0 && input->nn % 16 == 0){
			...
			quartaFaseQ(matriceSecondaTerzaFase,matriceV,input->out,input->n,input->nn, inizioFetta);
			...
			quartaFaseT(matriceSecondaTerzaFase,matriceV,input->out,input->n,input->nn, inizioFetta);
		}
			...
			quartaFaseD(matriceSecondaTerzaFase,matriceV,input->out,input->n,input->nn, inizioFetta);
		}
		else{
			...
			quartaFaseU(matriceSecondaTerzaFase,matriceV,input->out,input->n,input->nn, inizioFetta);
		}
		inizioFetta += 4 * input->n * input->nn;
	}
